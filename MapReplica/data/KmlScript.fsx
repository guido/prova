...
//piglia un elemento e ne trae un Placemark o nulla
...
let extractCodeNameAndCoords (p: Placemark) =
    ...
    let areaCode =
        GeoCode(codeData.Text |> int, siglaData.Text)
...
let province =
    "/province.kml" |> absolutePath |> readBoundaries


province |> Array.last

province
|> Array.map (fun ((GeoCode (numeric, sigla)), _, _) -> numeric)
|> Array.max
